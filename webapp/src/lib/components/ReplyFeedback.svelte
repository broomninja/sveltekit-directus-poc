<script lang="ts">
	...
	let showFeedbackReplyForm: boolean = false;
    ...
    const saveComment = async (data): Promise<any> =>  {

        const validation = schema.safeParse(data);

        if (!validation.success) {
            return new Promise((resolve, reject) => {
                const error = validation.error.flatten().formErrors[0];
                reject(error);
            });
        }
        ...
        const response = await fetch(`${activeUrl}/comment/new`, {
            method: 'POST',
...
{#if (isUserLoggedIn)}
    ...
    <label for="reply-feedback-toggle" class="cursor-pointer group relative inline-block text-sm ml-0 py-2 px-4 justify-center 
        items-center gap-2 rounded-md bg-blue-100 border border-transparent font-semibold text-blue-500 
        hover:text-white hover:bg-blue-500 focus:outline-none focus:ring-2 ring-offset-white focus:ring-blue-500 focus:ring-offset-2
        transition-all dark:focus:ring-offset-gray-800 dark:bg-gray-900 dark:hover:bg-blue-400 dark:text-white">
            Reply
    </label>

    <input id="reply-feedback-toggle" type="checkbox" class="sr-only peer/reply-toggle" bind:checked={showFeedbackReplyForm} />

    {#if showFeedbackReplyForm}
    <form
        in:fade
        method="POST"
        on:submit|preventDefault={handleReply} 
        class="flex-col hidden gap-4 mt-5 peer-checked/reply-toggle:flex md:flex-row md:items-start"
    >
        <textarea
            id="reply"
            name="content"
    ...
    text-gray-900 text-sm md:text-sm w-full outline-none ring-blue-400 focus-within:ring-1 hover:ring-1"
        ...
        <input type="hidden" name="feedbackId" bind:value={formData.feedbackId} />
        ...
        md:min-w-[120px]">
            ...
            <div>
                <button type="submit" disabled={!formData.content.trim()} 
                        class="disabled:opacity-30 mt-6 py-2 px-4 rounded-md bg-blue-100 border border-transparent text-sm font-semibold 
...
{:else}
    ...
    <a href={`/login/?redirectUrl=${activeUrl}`}>
        <span>
            <ButtonWithTooltip buttonText='Reply' 
